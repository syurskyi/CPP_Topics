...
in. main
	...
	c.. st. filesystem::path currentDir _ st. filesystem::current_path()
	st. c__ __  currentDir __  st. e..

	c.. st. filesystem::path someFile _ currentDir / "subfolder" / "readme.txt"
	st. c__ __  someFile __  st. e..

	c.. st. filesystem::path otherFile _ currentDir / "otherFolder/otherFile.txt"
	...
	c.. st. filesystem::path cyrillic _ currentDir / L"директория/файл.txt"
	st. c__ __  cyrillic __  st. e..
	st. c__ __  cyrillic.u8string() __  st. e..


	c.. st. filesystem::path folder _ currentDir / "folder"
	st. c__ __  "First exists: " __  st. filesystem::exists(folder) __  st. e..
	___ {
		st. filesystem::create_directories(folder)
	}
	...
	c.. st. filesystem::path wrongFolder _ "/not_permitted"

	st. error_code errCodesy.. p..
	st. filesystem::create_directories(wrongFolder, errCode)
	__ (errCode) {
		st. c__ __  "Creation failed! " __  errCode.message() __  st. e..
	}

	c.. st. filesystem::path file _ folder / "file.txt"
	{
		st. ofstream stream(file.native())
		__ (!stream.is_open()) {
